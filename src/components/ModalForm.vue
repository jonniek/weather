  ...
  <form v-on:submit.prevent="sendMeasurement" class="modal">
    <div class="formfield">
      ...
      <div class="rangewrap">
        <span
          class="increment"
          v-on:click="$store.dispatch('addTemperature', -1)"
        >-</span>
        <input
          class="range"
          @input="updateInput"
          ...
          type="range"
          :min="minmaxTemperature[0]"
          :max="minmaxTemperature[1]"
          step="1"
        />
        <span
          class="increment"
    ...
    <div class="submitContainer">
      <p
        class="warning brief"
        v-if="locationShouldWarn"
      >Your closest location is {{ closestLocation.name }}. Are you sure you mean to submit to {{ currentLocation.name }}?</p>
      <div
        :disabled="submitDisabled"
        class="modalButton"
        v-on:click="sendMeasurement"
      >Submit</div>
    </div>
  </form>
</template>
